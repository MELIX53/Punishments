...
class PunishCommandUnBan extends Command
{
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args)
    {
        ...
        if(!isset($args[0])){
            $sender->sendMessage(PunishManager::PREFIX . "Пишите §b/punban ник");
            return;
        }

        if(PunishManager::getInstance()->unBan(strtolower($args[0]), $sender->getName())) {

            $broadcastMessage = PunishManager::BROADCAST_MESSAGE_UNBAN;
            ...
            $sender->sendMessage(PunishManager::PREFIX . "Вы успешно сняли блокировку по нику в игрока §b{$args[0]}");
