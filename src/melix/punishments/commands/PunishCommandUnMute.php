...
class PunishCommandUnMute extends Command
{
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args)
    {
        ...
        if(!isset($args[0])){
            $sender->sendMessage(PunishManager::PREFIX . "Пишите §b/unmute ник");
            return;
        }

        if(PunishManager::getInstance()->unMute(strtolower($args[0]), $sender->getName())) {

            $broadcastMessage = PunishManager::BROADCAST_MESSAGE_UNMUTE;
            ...
            PunishManager::getInstance()->remMuteCache(strtolower($args[0]));
            $sender->sendMessage(PunishManager::PREFIX . "Вы успешно сняли блокировку чата в игрока §b{$args[0]}");
